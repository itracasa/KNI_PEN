# KPI_19 - Reducción anual de energía consumida
...
# KPI_19 - Reducción anual de energía consumida
# ---------------------
# Reducción anual de energía consumida.
# Comparación del consumo real por meses frente al consumo teórico según criterios CTE-2019
#
...
kpi = 'KPI_19'
# desde = format(datetime.datetime.today().replace(day=1) - datetime.timedelta(days = 366), '%Y-%m-%d')
# hasta = format(datetime.datetime.today().replace(day=1) - datetime.timedelta(days = 1), '%Y-%m-%d')
desde = desde()
hasta = hasta()
...
df['consumo_energetico_total_mensual']=df['primario_1']+df['primario_2'] # Consumo energético total mensual
df['mes']=df['fecha'].str.slice(5)

# Valores de consumo estándar según especificaciones de KPI_19
consumo_mes = { '01':0.16,'02':0.12,'03':0.115,'04':0.09,\
                '05':0.075,'06':0.04,'07':0.04,'08':0.04,\
                '09':0.04,'10':0.055,'11':0.09,'12':0.135 }
...
df['consumo_energetico_referencia'] = superficie * coeficiente_consumo * df['coef_cons_mes'] # Consumo energético de referencia
coste_energia = config.getfloat('kpi_19','coste_energia')
# v_01 = Ahorro energético mensual (%) respecto edificio de referencia
# v_02 = Ahorro económico mensual (€) respecto edificio de referencia
df['v_01'] = (df['consumo_energetico_referencia']-df['consumo_energetico_total_mensual'])/df['consumo_energetico_referencia']*100 # Ahorro energético respecto edificio de referencia
df['v_02'] = (df['consumo_energetico_referencia']-df['consumo_energetico_total_mensual']) * coste_energia # Ahorro en coste 
...
df[['edificio_id','kpi_id','fecha','v_01']].to_json(kpi_path , orient = 'split' , index = False)
